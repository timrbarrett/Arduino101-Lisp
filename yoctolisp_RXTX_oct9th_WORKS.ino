...
/*
  ...
  29th Sept - Sketch uses 92460 bytes (59%) of program storage space. Maximum is 155648 bytes. BLE, rx tx uart working.
  1st Oct - 107076 bytes (69%) shuld NULL be nullptr? do I need to redirect all printing to transmit? I can optimise PRogMem.
  8th Oct - 108612 bytes (69%) - send lisp over UART, fixable casting problem currently prevents response
  9th Oct - 108316 bytes (69% 
  to do: minimise serial output, test some more of the functions, tidy up code

*/

// porting it to Arduino 101 has taken cosiderably longer than a weekend
...
#define ul(x) Serial.println(x)
#define u(x) Serial.print(x)
#define OUTPUT_EVAL_STAGES 0
/* BLE */
...
{
  if (OUTPUT_EVAL_STAGES) ul("eval_variable");
  YLispValue *v;

  while (context != NULL) {
    //u("!Null");
    u(" var:"); ylisp_print(var); ul("");
...
{
  if (OUTPUT_EVAL_STAGES) ul("eval_func_args");
...
{
  if (OUTPUT_EVAL_STAGES) ul("eval_func_call");
...
{
  if (OUTPUT_EVAL_STAGES) ul("eval_list_inner");
...
{
  if (OUTPUT_EVAL_STAGES) ul("eval_list");
...
YLispValue *ylisp_eval(YLispValue *context, YLispValue *code)
{
  u("ylisp_eval code=["); ylisp_print(code); u("]"); //ul(string(lexer.position));
...
void BLEAppend(const char *text) {
  ...
  for (unsigned int i = 0; i < strlen((const char*)text); ++i) { // for every character in the message
    ...
    if (filledTo == BLE_MAX_LENGTH || text[i] == '\n') { // ready to transmit?

      //if (mirrorREPLToSerial) //
      Serial.print((char)text[i]);
...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if ((code = ylisp_parse(&lexer)) != NULL) {
        ylisp_print(code);
        Serial.println(">");
      ...
      if (outputMemoryOnce == true) {
        ...
        */
        ...
        transmit("Sizeof ylispcell:" + String(sizeof(YLispValue)));
      }
...
void rxCharacteristicWritten(BLECentral& central, BLECharacteristic& characteristic) {
  ...
  if (characteristic.value()) {
    ...
    // zero out any undefined characters
    //for (int idx2 = characteristiclength ; idx2 < 20 ; ++idx2 ) {
    //  rxText[idx2] = '\0';
    //}
